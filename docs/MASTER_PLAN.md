...
- [x] Phase 2 (self-hosted OSRM — owner-approved endpoint only): `measureOsrmRoute` runs behind `NEXT_PUBLIC_OSRM_URL` with named CSP origin (`connect-src` gains exactly that origin), 8s timeout fallback, OSM attribution rendered with every estimate, and the public demo server (`router.project-osrm.org`, incl. subdomains) rejected at URL-build time and NEVER fetched; measurement only displays km/min and never reorders stops (manual order kept on any error); documented in `.env.example`; covered by `routeEngine.test.ts` (not-configured/demo-ban/invalid-input/success/invalid-response/network/timeout)
- [x] Daily route operations framework documented: repeatable import → validation → evidence-based enrichment → geographic grouping → route-lite review → bilingual driver sheet → close; uncertainty and model-worker boundaries are explicit in `docs/ROUTE_OPERATIONS_FRAMEWORK.md`
- [x] Logestechs boundary documented from the official docs/collection pointers: public endpoint inventory is research evidence only; credentials, tenant scope, schemas, and write authority remain unverified and blocked in `docs/LOGESTECHS_INTEGRATION_BOUNDARY.md`
- [x] Optional local `pi` worker policy documented: advisory-only, bounded snapshots, no coordinates/distances/driver decisions/write-back, and no dependency on model availability (`docs/PI_WORKER_POLICY.md`)
- [x] Driver handoff aid added: each assigned run exposes a multi-stop Google Maps driving link and a UTF-8 CSV download in reviewed stop order; links are navigation aids only and do not claim measured road distance
- [x] Daily input aid added: `.xlsx`/`.xls` first-sheet rows enter the existing preview, validation, duplicate, and explicit-confirmation path; no spreadsheet row bypasses safeguards
- [x] Two-driver geographic proposal added: deterministic coordinate clustering with missing-coordinate disclosure, review-before-assign, depot/start-point input, and optional return-to-depot Google Maps link
