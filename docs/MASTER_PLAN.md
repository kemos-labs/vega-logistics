...
- [ ] Phase 2 (needs owner approval + coordinate coverage): enable and accept self-hosted OSRM behind `NEXT_PUBLIC_OSRM_URL` with named CSP origin, OSM attribution visible, timeout fallback, demo endpoint banned for production; the opt-in adapter and UI measurement path are now implemented but inert by default
