...
- Dev = http://vega.localhost:8080 (portless alias; localhost:3002 belongs to the Options Terminal project — never cite it here) · Pages deploy green · tsc clean · all tests passing (187: incl. 45 backup + 25 parser) · ESLint 0/0
...
## Current phase
**P1 backup core DONE through review contracts C→E plus stabilization F; **P1 COMPLETE: core ACCEPTED, banner shipped (G da133b8), WhatsApp parser shipped (H).**
H additions: `src/lib/providerMessageParser.ts` pure deterministic engine — digit normalization (Arabic-Indic+Persian), term dictionary w/ spelling variants (سيارة/سياره، لوحة/لوحه، تحميل/محمل، توصيل/تم التوصيل/تمت التوصيل، راجع/مرتجع), name = free-text before first field term (numeric-only ⇒ ambiguous warning), missing any of loaded/delivered/returned NUMBERS ⇒ rejected (no phantom zeros); reconcile() exposes loaded−(delivered+returned) verbatim; applyPreviewToRecord writes ONLY reviewed fields (completed=delivered, failed=returned, driver/car/plate) preserving all unrelated record fields. UI: ProviderImportCard in Daily view — paste→parse→preview grid→mismatch blocks confirm→existing-date overwrite needs explicit ack→confirm merges via parent setDailyRecords.
